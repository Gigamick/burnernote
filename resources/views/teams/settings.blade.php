...
@section('content')
    <div class="max-w-2xl mx-auto px-4 sm:px-6">
        ...
        <form method="POST" action="{{ route('teams.settings.update', $team) }}" id="team-settings-form" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6 transition-colors duration-200">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <span id="general-saved" class="text-sm text-green-600 dark:text-green-400 opacity-0 transition-opacity duration-200">Saved</span>
                </div>
                <div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6 transition-colors duration-200">
                <div class="flex justify-between items-center mb-4">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Security Policies</h2>
                    <span id="policies-saved" class="text-sm text-green-600 dark:text-green-400 opacity-0 transition-opacity duration-200">Saved</span>
                </div>
        ...
        <script>
            let teamSettingsTimeout;

            function debounceTeamSettings() {
                clearTimeout(teamSettingsTimeout);
                teamSettingsTimeout = setTimeout(() => saveTeamSettings(), 800);
            }
            ...
            function saveTeamSettings() {
                ...
                .then(data => {
                    if (data.success) {
                        showSaved('general-saved');
                        showSaved('policies-saved');
                    }
                });
            }
